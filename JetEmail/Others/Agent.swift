...
extension TreeNode<MSGraph.MailFolder> {
    var path: [String] {
...
extension Agent {
    static func classify(archiveNode: TreeNode<MSGraph.MailFolder>, junkNode: TreeNode<MSGraph.MailFolder>, message: MSGraph.Message) async throws -> String? {
        ...
        return (try? response.choices[0].message.toolCalls?.first?.function.arguments?.jsonDecode(ClassifyResult.self))?.existedFolder
    }
}
