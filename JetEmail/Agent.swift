...
extension Agent {
    static func classify(archiveNode: TreeNode<Microsoft.Graph.MailFolder>, junkNode: TreeNode<Microsoft.Graph.MailFolder>, message: Microsoft.Graph.Message) async throws -> String? {
        ...
        return response.choices[0].message.toolCalls?.first?.function.arguments?.decodeJSON(ClassifyResult.self)?.existedFolder
    }
}
