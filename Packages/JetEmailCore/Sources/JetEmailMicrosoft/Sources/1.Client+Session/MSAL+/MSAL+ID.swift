...
extension MSALAccount {
    var id: MicrosoftAccountID { get throws {
        guard let id = identifier else { throw MicrosoftAuthError.accountNoIDOrUsername }
        return .init(innerID: id)
    } }
    
    var outer: MicrosoftAccount { get throws {
        guard let id = identifier, let username else { throw MicrosoftAuthError.accountNoIDOrUsername }
